...
# Ohne Zuweisung der Gemeinden - Wie haben sich die Ausgaben gesamt entwickelt?
...
# Nach Gemeindegröße die Entwicklung der Personalkosten von fusionierten und nicht-fusionierten ansehen
...
options(scipen=999)
p <- ggplot(foerderungen_parteien_gem, aes(fj, sum, group=blcode, color=blcode)) +
  geom_line() 
print(p)
...
foerderungen_parteien_gem_avg_allyears_dw <- posten_data %>%
  filter(ans2=="00" & post3=="757" & fh=="2017") %>%
  filter (hh=="1" | hh=="5") %>%
  mutate(sollproew = round(soll/ew, 1)) %>%
  ungroup()

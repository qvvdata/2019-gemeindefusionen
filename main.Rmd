...
personalkostenprokopf <- posten_data %>%
  ...
  filter (hh=="1" | hh=="5") %>% # auf welchen Ansatz/Haushalt muss hier gefiltert werden? --> Gabriel fragen! Aktuell keine Filterung
...
#Visualisierung der Personalkosten pro Kopf mit Quantilen für die Kleine Zeitung
q <- ecdf(personalkostenprokopf_map$avg15_17)

p <- ggplot() +
  geom_sf(
    data= (personalkostenprokopf_map %>%
             mutate(
               gkz=as.character(gkz_neu),
               q=round(q(avg15_17)*3)+1) %>%
             left_join(gde_18, by = c("gkz" = "GKZ"))),
    aes(fill=q), color="white", size=0.01) +
  coord_sf(datum=sf::st_crs(4326))
print(p)

quantile(personalkostenprokopf_map$avg15_17, probs = seq(0, 1, by = 0.10))

# Maximum für höchsten Decil setzen in der Visualisierung
personalkostenprokopf_map$avg15_17[personalkostenprokopf_map$avg15_17 >= 464] <- 464
...
plot_1 <- ggplot() +
  ...
  labs(x = NULL, y = NULL, title = "personalkostenprokopf", caption = "Quelle: Statistik Austria, BEV.") +
...
# # Ohne Zuweisung der Gemeinden - Wie haben sich die Ausgaben gesamt entwickelt? --> Falsch, weil bevsumm!
# personalkostenprokopfstmkggesamt <- posten_data %>%
#   filter(gruppe=="Personal") %>%
#   filter (hh=="1" | hh=="5") %>% 
#   filter(gkz_neu >60000 & gkz_neu < 70000) %>%
...
#   summarise(personalausgaben = sum(soll), 
#             bevsumme = sum(ew)) %>%
#   mutate(personalausgabenprokopf = round(personalausgaben/bevsumme,2)) %>%
#     ungroup() %>%
#   select(fj, gsrbetr, personalausgabenprokopf) %>%
#   spread(fj, personalausgabenprokopf) %>%
#   mutate(avg12_14 = (`2012`+`2013`+`2014`)/3, 
#          avg15_17 = (`2015`+`2016`+`2017`)/3, 
#          avg_diff = (avg15_17/avg12_14-1)*100, 
#          avg_diff17 = (`2017`/`2016`-1)*100)

# Gesamtentwicklung absolut --> falsch weil bevsum
# personalkostenprokopfstmkggesamtabs <- posten_data %>%
#   filter(gruppe=="Personal") %>%
#   filter (hh=="1" | hh=="5") %>% 
#   filter(gkz_neu >60000 & gkz_neu < 70000) %>%
#   group_by(fj,gsrbetr, gruppe) %>%
...
#             bevsumme = sum(ew)) %>%
#     ungroup() %>%
#   select(fj, gsrbetr, personalausgaben) %>%
...
# Nach Gemeindegröße die Entwicklung der Personalkosten von fusionierten und nicht-fusionierten ansehen --> Falsch weil bevsum
# personalkostenprokopfstmkgemgr <- posten_data %>%
#   filter(gruppe=="Personal") %>%
#   filter (hh=="1" | hh=="5") %>% 
#   filter(gkz_neu >60000 & gkz_neu < 70000) %>%
#   group_by(fj,gsrbetr, gruppe, gemgrklas) %>%
#   summarise(personalausgaben = sum(soll), 
#             bevsumme = sum(ew)) %>%
#   mutate(personalausgabenprokopf = round(personalausgaben/bevsumme,2)) %>%
#     ungroup()
# 
# p <- ggplot(personalkostenprokopfstmkgemgr, aes(x = fj, y = personalausgabenprokopf, color=gsrbetr, group=gsrbetr)) +
#     geom_line(lwd=0.9) +
#     facet_wrap(~gemgrklas)
# 
# print(p)
# ggsave("output/ignore/p.pdf", width = 20, height = 20, units = "cm")
...
```
42/128 gesunkene Kosten für Referate pro Kopf (40 %) bei fusionierten
...
65/128 (50 %) gesunkene Kosten pro Kopf für Feuerwehren in fusionierten Gemeinden
...
gemeindeorganeprokopf_nfus <- gemeindeorganeprokopf %>%
  filter(gsrbetr=="Nein")  
```
106/128 (84 %) gesunkene Kosten pro Kopf für gewählte Gemeindeorgane in fusionierten Gemeinden
0/159 gesunkene Kosten pro Kopf für gewählte Gemeindeorgane in nicht fusionierten Gemeinden
...
# Hat die politische Partizipation in den Wahlen nach der Fusion abgenommen?
# Hat die politische Repräsentation abgenommen? Wie viele Gemeinderäte kommen nun auf die Bürger? Studie: Hat die Kleine zeitung darüber berichtet? Sollen wir das ein Jahr davor machen: Nach der Reform, vor der Wahl: Ihr Stimme ist so wenig wert wie nie
# Können wir den Common-Pool-Effekt ( Noch schnell was großes anschaffen, damit wir es auf die große Gemeinde abwälzen können, nachweisen mit den außerordentlichen ausgaben?)
...
verwaltungsausgabenprokopf_t <- verwaltungsausgabenprokopf %>%
...
test <- rbind(verwaltungsausgabenprokopf_t, verwaltungsausgabenprokopf_vfgh)

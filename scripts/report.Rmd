...
map_data <- verwaltungsausgaben_gemeinden %>% mutate(gkz_neu=as.character(gkz_neu)) %>% left_join(gde_18_2, by = c("gkz_neu" = "GKZ")) 
...
map_1 <- ggplot() +
  geom_sf(data=bezirksgrenzen_2 %>%
      ...
      filter(regionalausgabe==ausgewaehlte_regionalausgabe),
    fill='lightyellow') +
  geom_sf(data = map_data, aes(fill=`Differenz in %`), color="white", size=0.001, datum=sf::st_crs(4326)) +
  coord_sf() +

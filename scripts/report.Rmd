...
# Gemeindestrukturreform für die Lokalausgabe `r regionalausgabe`
...
```{r echo=F, warning=F}
regionalausgabe_ew <- verwaltungsausgaben %>%
...
map_data <- verwaltungsausgaben_gemeinden %>% mutate(gkz_neu=as.character(gkz_neu)) %>% left_join(gde_18, by = c("gkz_neu" = "GKZ")) 
library(ggthemes)
map_1 <- ggplot() +
  # municipality polygons & outline
  geom_sf(data = map_data, aes(fill=`Differenz in %`), color="white", size=0.001) +
...
```{r echo=F, warning=F}
options(knitr.table.format = "html")
# Bevölkerungsveränderung seit 1971 
kable(data_groessteverluste_bl71) %>%
